...
import Link from 'next/link'
import dynamic from 'next/dynamic'
import Catalog from '@/components/Catalog' // 引入 Catalog 组件
const ExampleRecentComments = dynamic(() => import('./ExampleRecentComments'))

export const SideBar = (props) => {
  const { locale, post } = useGlobal() // 假设 post 对象从 useGlobal 返回
  const { latestPosts, categories } = props
  return (
      <div className="w-full md:w-64 sticky top-8">
            ...
            {/* 添加目录组件 */}
            <aside className="rounded shadow overflow-hidden mb-6">
              <h3 className="text-sm bg-gray-100 text-gray-700 dark:bg-hexo-black-gray dark:text-gray-200 py-3 px-4 dark:border-hexo-black-gray border-b">目录</h3>
              <div className="p-4">
                <Catalog post={post} />
              </div>
            </aside>
